 ...
 */
export function buildExpireNs(durationMs) {
    if (durationMs <= 0) {
...
export function assertExpireNs(expireNs) {
    const nowNs = BigInt(Date.now()) * NS_PER_MS;
    if (expireNs <= nowNs) {
        throw new GotchaError("EXPIRE_NS_NOT_FUTURE", `expireTimestampNs must be strictly in the future (got ${expireNs}, now=${nowNs}). ` +
 ...
 */
export function assertPriceRawNonZero(priceRaw) {
    if (priceRaw <= 0n) {
        throw new GotchaError("PRICE_RAW_ZERO", `priceRaw must be > 0 (got ${priceRaw}). priceRaw = 0 is a literal price on DreamDEX, ` +
 ...
 */
export function assertBuilderDisabled(builder, builderFeeBpsTimes1k) {
    if (builder.toLowerCase() !== zeroAddress) {
        throw new GotchaError("BUILDER_NOT_ZERO", `builder must be the zero address on this kit's untagged order path (got ${builder}). ` +
            `Builder codes exist on mainnet (1% cap) but this kit does not tag orders yet.`);
    }
    if (builderFeeBpsTimes1k !== 0n) {
        throw new GotchaError("BUILDER_FEE_NOT_ZERO", `builderFeeBpsTimes1k must be 0 when builder is the zero address (got ${builderFeeBpsTimes1k}).`);
    }
}
export function assertQtyMultipleOfLot(qtyRaw, lotRaw) {
    if (lotRaw <= 0n)
        throw new GotchaError("LOT_RAW_ZERO", "lotRaw must be > 0.");
    if (qtyRaw % lotRaw !== 0n) {
        throw new GotchaError("QTY_NOT_LOT_MULTIPLE", `quantity ${qtyRaw} is not a whole multiple of lotSize ${lotRaw}. Use quant.alignToLot().`);
    }
}
export function assertQtyAboveMin(qtyRaw, minQtyRaw) {
    if (qtyRaw < minQtyRaw) {
        throw new GotchaError("QTY_BELOW_MIN", `quantity ${qtyRaw} is below the market minimum ${minQtyRaw}. ` +
            `minQuantity is the #1 cause of a rejected first order.`);
    }
}
export function assertPriceMultipleOfTick(priceRaw, tickRaw) {
    if (tickRaw <= 0n)
        throw new GotchaError("TICK_RAW_ZERO", "tickRaw must be > 0.");
    if (priceRaw % tickRaw !== 0n) {
        throw new GotchaError("PRICE_NOT_TICK_MULTIPLE", `price ${priceRaw} is not a whole multiple of tickSize ${tickRaw}. Use quant.alignToTick().`);
    }
}
